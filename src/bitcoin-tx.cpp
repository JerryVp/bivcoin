...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bivcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bivcoin-tx [options] <hex-tx> [commands]  Update hex-encoded bivcoin transaction\n" +
            "or:     bivcoin-tx [options] -create [commands]   Create hex-encoded bivcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bivcoin transaction
