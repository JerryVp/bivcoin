...
const char * const BITCOIN_CONF_FILENAME = "bivcoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bivcoin-tx using stdin
        std::string val;
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bivcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bivcoin
    // Mac: ~/Library/Application Support/Bivcoin
    // Unix: ~/.bivcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bivcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bivcoin";
#else
    // Unix
    return pathRet / ".bivcoin";
#endif
#endif
}
