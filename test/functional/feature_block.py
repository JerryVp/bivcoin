...
class FullBlockTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create a transaction that spends one yamatona to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 yamatona to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        #                        \-> b68 (20)
        #
        # b68 - coinbase with an extra 10 yamatonas,
        #       creates a tx that has 9 yamatonas from out[20] go to fees
        #       this fails because the coinbase is trying to claim 1 yamatona too much in fees
        #
        # b69 - coinbase with extra 10 yamatonas, and a tx that gives a 10 yamatona fee
        #       this succeeds
        #
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), *, version=1):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one yamatona to fees
            coinbase.rehash()
            block = create_block(base_block_hash, coinbase, block_time, version=version)
            tx = self.create_tx(spend, 0, 1, script)  # spend 1 yamatona
            self.sign_tx(tx, spend)
