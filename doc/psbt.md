# PSBT Howto for Bivcoin Core

Since Bivcoin Core 0.17, an RPC interface exists for Partially Signed Bivcoin
...
PSBT is an interchange format for Bivcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Bivcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Bivcoin transaction (in network format)
...
#### Multisig with multiple Bivcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Bivcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bivcoin-cli` is used.
...
Later, when *V* BIV has been received on *Amulti*, and Bob and Carol want to
