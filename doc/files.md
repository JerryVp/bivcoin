...
The data directory is the default location where the Bivcoin Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.bivcoin/`
macOS    | `$HOME/Library/Application Support/Bivcoin/`
Windows  | `%APPDATA%\Bivcoin\` <sup>[\[1\]](#note1)</sup>

2. The non-default data directory path can be specified by `-datadir` option.

3. All content of the data directory, except for `bivcoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Bivcoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `bivcoin.conf`        | Contains [configuration settings](bivcoin-conf.md) for `bivcoind` or `bivcoin-qt`; can be specified by `-conf` option
`./`               | `bivcoind.pid`        | Stores the process ID (PID) of `bivcoind` or `bivcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `bivcoind` or `bivcoin-qt`; can be specified by `-debuglogfile` option
...
These subdirectories and files are no longer used by the Bivcoin Core:
